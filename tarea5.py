...
def readFiles(v,r,host, users, passwords):
	'''
		...
		for u in usert.splitlines():
			if result== True:
				...
				for p in passwdt.splitlines():
					result= makeRequest(host,u,p,v,r)
...
def makeRequest(host, user, password, v,r):
    '''
...
if __name__ == '__main__':
	try:
		...
		if opts.users:
			readFiles(opts.verbose,opts.reporte,url,opts.users,opts.passwords)
		else:
			makeRequest(url, opts.user, opts.password, opts.verbose, opts.reporte)
